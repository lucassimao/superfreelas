...
describe("Freela's API test", () => {
    ...
    test('It should answer the GET method to / with the newest available freelas ', () => {
        return request(app)
            ...
            .expect(res => {
                ...
                expect(res.body.freelas.length).toBeLessThanOrEqual(
                    FreelaController.PAGE_SIZE
                )
            })
    })
    ...
    test('GET to /freelas/:country/:state/:city/:uri should return all freelas available for that city on that state on that country with such uri', () => {
        return request(app)
            .get(
                '/freelas/br/piaui/teresina/anuncio-garçon-temporário-churrascaria-123'
            )
            .expect('Content-Type', /json/)
            .expect(200)
            .expect(async res => {
                ...
                expect(res.body.uri).toBe(
                    'anuncio-garçon-temporário-churrascaria-123'
                )
    ...
    test('POST to /freelas should return the http status 201 if it was able to insert a new Freela', () => {
        const freela = {
            ...
            description:
                'I have 6 doors in my apartment which need to be painted',
            priceRangeStart: 100,
            priceRangeEnd: 200,
            currency: 'R$',
        }
